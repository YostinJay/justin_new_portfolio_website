...
class EvolveThemes_Module_ThemePage {
	...
	public function add_theme_page() {
		...
		if ( $notices_count > 0 ) {
			...
			$update_label = sprintf( _n( '%1$s notice', '%1$s notices', $notices_count, 'schism' ), $notices_count );
			...
			$menu_item_title = sprintf( esc_html__( 'About %1$s %2$s', 'schism' ), $theme_data->get( 'Name' ), wp_kses_post( $count ) );
		} else {
			/* translators: %1$s: The theme name */
			$menu_item_title = sprintf( esc_html__( 'About %1$s', 'schism' ), $theme_data->get( 'Name' ) );
		}

		$menu_item_title = evolvethemes_apply_filters( 'theme_page_menu_title', $menu_item_title );

		/* translators: %1$s: The theme name */
		$menu_page_title = sprintf( esc_html__( 'About %1$s', 'schism' ), $theme_data->get( 'Name' ) );
		$menu_page_title = evolvethemes_apply_filters( 'theme_page_menu_page_title', $menu_page_title );
	...
	public function enqueue() {
		wp_enqueue_style( 'evolvethemes-theme-page-css', EV_INC_FOLDER_URI . 'modules/theme-page/v1/css/theme-page.min.css', array(), '1.0.0' );
	}
