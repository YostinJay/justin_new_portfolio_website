...
function evolvethemes_theme_page_structure() {
	...
	$theme_page_title = sprintf( __( 'Welcome to %1$s - Version %2$s', 'schism' ), $theme_data->get( 'Name' ), $theme_data->get( 'Version' ) );
	$theme_page_title = evolvethemes_apply_filters( 'theme_page_title', $theme_page_title );

	$about_theme = '';
	$about_theme = evolvethemes_apply_filters( 'theme_page_about', $about_theme );
	...
	$badge = evolvethemes_apply_filters( 'theme_page_badge', $badge );

	$tab_slug = null;

	if ( isset( $_GET['tab'] ) ) { // phpcs:ignore
		$tab_slug = sanitize_text_field( wp_unslash( $_GET['tab'] ) ); // phpcs:ignore
	} else {
		$tab_slug = null;
	}
...
function evolvethemes_get_notices() {
	...
	$notices = evolvethemes_apply_filters( 'get_notices', $notices );
...
function evolvethemes_get_tabs() {
	$tabs = array();
	$tabs = evolvethemes_apply_filters( 'theme_page_tabs', $tabs );
