...
function evolvethemes_system_status() {
	...
	$status['wp_multisite'] = array(
		'value' => is_multisite() ? __( 'On', 'schism' ) : __( 'Off', 'schism' ),
	);

	$status['wp_memory_limit'] = array(
		'value'   => $memory,
		/* translators: codex page for memory. */
		'warning' => intval( $memory ) < 64 ? sprintf( __( 'We recommend setting memory to at least 64MB. See <a href="%s" target="_blank" rel="noopener noreferrer">official WordPress documentation</a>.', 'schism' ), esc_attr( $memory_codex_url ) ) : '',
	);

	$status['wp_debug'] = array(
		'value'   => $debug ? __( 'On', 'schism' ) : __( 'Off', 'schism' ),
		/* translators: codex page for debug. */
		'warning' => $debug ? sprintf( __( 'Debug mode is not suitable for production use, you might want to turn it off. See <a href="%s" target="_blank" rel="noopener noreferrer">official WordPress documentation</a>.', 'schism' ), esc_attr( $debug_codex_url ) ) : '',
	);

	/* Server-side checks. */
	$status['php_version'] = array(
		'value'   => $php_version,
		'warning' => version_compare( $php_version, '5.6', '<' ) ? __( 'Although the theme is compatible with PHP 5.2+, we recommend a minimum PHP version of 5.6.', 'schism' ) : '',
	);
	...
	$status['php_time_limit'] = array(
		...
		'value' => sprintf( _x( '%s seconds', 'php time limit', 'schism' ), $php_time_limit ),
	);
	...
	$status['theme_child'] = array(
		'value' => is_child_theme() ? __( 'On', 'schism' ) : __( 'Off', 'schism' ),
	);
